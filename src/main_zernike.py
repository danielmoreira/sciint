...
from core import alg_single, alg_double, get_colored, alg_match_single, alg_match_double, clip_padding, clip_pannels_padding, FE_PM
from det_subpanels import get_mask, subpanel_segmentation
from text_detector import TextNet

def alg_single_v3(input_img, param = dict()):
    ...
    feat, padsize, diameter = FE_PM(img2grayf(input_img.copy()), param=param)
    msk_text = clip_padding(msk_text, padsize)
    panels = clip_pannels_padding(panels, padsize)
        ...
        for index2 in range(num_panels):
            ...
            if index2==index1:
                ...
                msk1, msk2 = alg_match_double(feats[index1], feats[index2],
                     input_maskA = msks_text[index1], input_maskB = msks_text[index2],
                     param = param)
...
def alg_double_v3(input_imgA, input_imgB, param = dict()):
    param['remove_panel'] = False
    param = setParameters(param, 'subpanel_background_th'     , 0.98)
    param = setParameters(param, 'subpanel_foreground_th_low' , 0.95)
    param = setParameters(param, 'subpanel_foreground_th_high', 0.70)
    ...
    # Feature Extraction & Matching
    featA, padsizeA, diameterA = FE_PM(img2grayf(input_imgA.copy()), param=param)
    featB, padsizeB, diameterB = FE_PM(img2grayf(input_imgB.copy()), param=param)
    msk_textA = clip_padding(msk_textA, padsizeA)
    msk_textB = clip_padding(msk_textB, padsizeB)
    panelsA = clip_pannels_padding(panelsA, padsizeA)
    panelsB = clip_pannels_padding(panelsB, padsizeB)
...
if __name__=="__main__":
    ...
    if len(sys.argv)==3:
        ...
        maskA, maskB = alg_double_v3(imgA, imgB)
