...
COPY ./ /provenance
WORKDIR /provenance
RUN chmod 775 /provenance/build_graphs.sh
RUN chmod 775 /provenance/eval_graphs.sh

# Creates the input-output folder (it must be mounted as a volume when executing this container).
